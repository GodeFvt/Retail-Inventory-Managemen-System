...
import { AUTH_COOKIE_NAME, SESSION_MAX_AGE_SECONDS } from "@/app/api/auth/_lib/constants";
import { getJwtSecret, verifySessionToken, type SessionTokenPayload } from "@/app/api/auth/_lib/edge-session";
...
async function signSessionToken(payload: SessionTokenPayload, expiresAt: Date) {
  return new SignJWT({
